...
function Tabs:Create(name)
    ...
    B.MouseButton1Click:Connect(function()
        ...
        for _, v in pairs(TabContainer:GetChildren()) do
            if v:IsA("TextButton") then v.TextColor3 = Color3.fromRGB(180, 180, 180); v.BackgroundColor3 = Color3.fromRGB(30, 30, 35) end
        end
        P.Visible = true; B.TextColor3 = Color3.new(1, 1, 1); B.BackgroundColor3 = Color3.fromRGB(60, 60, 70)
    end)

    local El = {}
    function El:AddToggle(text, callback)
        local F = Instance.new("Frame", P)
        F.Size = UDim2.new(1, -5, 0, 38); F.BackgroundColor3 = Color3.fromRGB(25, 25, 30)
        Instance.new("UICorner", F).CornerRadius = UDim.new(0, 4)
        local L = Instance.new("TextLabel", F)
        L.Size = UDim2.new(1, -60, 1, 0); L.Position = UDim2.new(0, 10, 0, 0); L.BackgroundTransparency = 1
        L.Font = Enum.Font.GothamSemibold; L.Text = text; L.TextColor3 = Color3.new(1, 1, 1); L.TextSize = 12; L.TextXAlignment = Enum.TextXAlignment.Left
        local T = Instance.new("TextButton", F)
        T.Size = UDim2.new(0, 30, 0, 16); T.Position = UDim2.new(1, -40, 0.5, -8); T.BackgroundColor3 = Color3.fromRGB(50, 50, 60); T.Text = ""
        Instance.new("UICorner", T).CornerRadius = UDim.new(1, 0)
        local D = Instance.new("Frame", T)
        D.Size = UDim2.new(0, 12, 0, 12); D.Position = UDim2.new(0, 2, 0.5, -6); D.BackgroundColor3 = Color3.new(1, 1, 1)
        Instance.new("UICorner", D).CornerRadius = UDim.new(1, 0)
        local s = false
        T.MouseButton1Click:Connect(function()
            s = not s
            TweenService:Create(D, TweenInfo.new(0.15), {Position = s and UDim2.new(0, 16, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)}):Play()
            TweenService:Create(T, TweenInfo.new(0.15), {BackgroundColor3 = s and Color3.new(1, 1, 1) or Color3.fromRGB(50, 50, 60)}):Play()
            TweenService:Create(D, TweenInfo.new(0.15), {BackgroundColor3 = s and Color3.new(0, 0, 0) or Color3.new(1, 1, 1)}):Play()
            callback(s)
        end)
    end
    function El:AddInput(text, callback)
        local F = Instance.new("Frame", P)
        F.Size = UDim2.new(1, -5, 0, 40); F.BackgroundColor3 = Color3.fromRGB(25, 25, 30)
        Instance.new("UICorner", F).CornerRadius = UDim.new(0, 4)
        local L = Instance.new("TextLabel", F)
        L.Size = UDim2.new(0.6, 0, 1, 0); L.Position = UDim2.new(0, 10, 0, 0); L.BackgroundTransparency = 1
        L.Font = Enum.Font.GothamSemibold; L.Text = text; L.TextColor3 = Color3.new(1, 1, 1); L.TextSize = 12; L.TextXAlignment = Enum.TextXAlignment.Left
        local Box = Instance.new("TextBox", F)
        Box.Size = UDim2.new(0, 70, 0, 24); Box.Position = UDim2.new(1, -80, 0.5, -12); Box.BackgroundColor3 = Color3.fromRGB(40, 40, 45); Box.Font = Enum.Font.GothamBold
        Box.TextColor3 = Color3.new(1, 1, 1); Box.TextSize = 12; Box.Text = ""; Box.PlaceholderText = "..."
        Instance.new("UICorner", Box).CornerRadius = UDim.new(0, 4)
        Box.FocusLost:Connect(function() callback(Box.Text) end)
    end
    function El:AddButton(text, callback)
        local B = Instance.new("TextButton", P)
        B.Size = UDim2.new(1, -5, 0, 32); B.BackgroundColor3 = Color3.fromRGB(40, 40, 45); B.Font = Enum.Font.GothamBold; B.Text = text; B.TextColor3 = Color3.new(1, 1, 1); B.TextSize = 12
...
P1:AddToggle("Hitbox Expander", function(v) Config.Hitbox = v end)
...
P1:AddInput("Range Aura", function(v) Config.KillauraRange = tonumber(v) or 25 end)
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            ...
            if hrp then
                if Config.Hitbox then
                    ...
                    if hrp.Size.X ~= 2 then
                        hrp.Size = Vector3.new(2, 2, 1); hrp.Transparency = 1; hrp.Massless = false
                    end
                end
            end
            
            if Config.ESP and not p.Character:FindFirstChild("QT_H") then
                local h = Instance.new("Highlight", p.Character); h.Name = "QT_H"; h.FillColor = Color3.new(1, 1, 1)
            ...
            if Config.ESPName and p.Character:FindFirstChild("Head") and not p.Character.Head:FindFirstChild("QT_N") then
                ...
                local t = Instance.new("TextLabel", b); t.Size = UDim2.new(1, 0, 1, 0); t.BackgroundTransparency = 1; t.Font = Enum.Font.GothamBold; t.TextColor3 = Color3.new(1, 1, 1); t.TextSize = 10; t.Text = p.DisplayName
            elseif not Config.ESPName and p.Character:FindFirstChild("Head") and p.Character.Head:FindFirstChild("QT_N") then
                p.Character.Head.QT_N:Destroy()
            end
        end
    end
...
OpenBtn.Size = UDim2.new(0, 45, 0, 45); OpenBtn.Position = UDim2.new(0, 10, 0.4, 0); OpenBtn.BackgroundColor3 = Color3.fromRGB(15, 15, 18); OpenBtn.Text = "QT"; OpenBtn.Font = Enum.Font.GothamBold; OpenBtn.TextColor3 = Color3.new(1, 1, 1); OpenBtn.TextSize = 16
Instance.new("UICorner", OpenBtn).CornerRadius = UDim.new(1, 0)
local OStroke = Instance.new("UIStroke", OpenBtn); OStroke.Thickness = 2; OStroke.Color = Color3.new(1, 1, 1)
